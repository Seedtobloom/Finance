...
<div id="app">
  ...
  <aside id="sidebar">
    <div class="sidebar-logo">
      ...
      <span style="display:block;font-size:10px;letter-spacing:.04em;color:var(--text-2);opacity:.7;margin-top:2px;">build v122 · Clarte des deux repartitions : le bloc « Ou va mon argent — repartition du disponible » (Combien me verser) est retitre et mis en avant ; un renvoi « Repartir ce disponible » est ajoute sous le disponible dans Ma tresorerie pour le retrouver facilement. Emplacement inchange (reste sur Combien me verser, choix valide).</span>
    </div>
...
function loadVersement(){
  ...
  const partVers=dispo>0?Math.round(dispo*pctVers*100)/100:0;
  ...
  const suiviHtml=\`<div class="col-12"><div class="card" style="padding:24px 26px;">
    <div style="display:flex;justify-content:space-between;align-items:baseline;flex-wrap:wrap;gap:10px;margin-bottom:4px;">
      <span style="font-size:13.5px;text-transform:uppercase;letter-spacing:.06em;color:var(--terre-400);font-weight:600;"><i class="ti ti-history"></i> Ce que je me verse · suivi</span>
  ...
  const repInner=dispo>0
    ? \`<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:12px;">
        <div style="background:var(--glycine);border-radius:12px;padding:14px 16px;">
          <div style="font-size:12px;color:var(--bleu);font-weight:600;"><i class="ti ti-cash"></i> Versement perso (\${Math.round(pctVers*100)}%)</div>
          <div style="font-family:'Cormorant Garamond',serif;font-style:italic;font-size:27px;color:var(--navy);line-height:1.1;">\${fmt(resteVirer)}</div>
          <div style="font-size:11.5px;color:var(--bleu);">reste à te virer ce mois\${curM.verse>0?' · déjà versé '+fmt(curM.verse):''} · part totale \${fmt(partVers)}</div>
        </div>
        <div style="background:var(--surface-2);border-radius:12px;padding:14px 16px;">
          <div style="font-size:12px;color:var(--terre-400);font-weight:600;"><i class="ti ti-building-bank"></i> Trésorerie (\${Math.round(pctTreso*100)}%)</div>
          <div style="font-family:'Cormorant Garamond',serif;font-style:italic;font-size:27px;color:var(--navy);line-height:1.1;">\${fmt(Math.round(dispo*pctTreso))}</div>
        </div>
        <div style="background:var(--surface-2);border-radius:12px;padding:14px 16px;">
          <div style="font-size:12px;color:var(--terre-400);font-weight:600;"><i class="ti ti-books"></i> Formation (\${Math.round(pctForm*100)}%)</div>
          <div style="font-family:'Cormorant Garamond',serif;font-style:italic;font-size:27px;color:var(--navy);line-height:1.1;">\${fmt(Math.round(dispo*pctForm))}</div>
        </div>
        \${pctEp>0.001?\`<div style="background:var(--surface-2);border-radius:12px;padding:14px 16px;">
          <div style="font-size:12px;color:var(--terre-400);font-weight:600;"><i class="ti ti-coin"></i> Épargne (\${Math.round(pctEp*100)}%)</div>
          <div style="font-family:'Cormorant Garamond',serif;font-style:italic;font-size:27px;color:var(--navy);line-height:1.1;">\${fmt(Math.round(dispo*pctEp))}</div>
        </div>\`:''}
      </div>\`
    : '<div style="font-size:13.5px;color:var(--text-2);padding:8px 0;">Aucun disponible à répartir pour le moment (tes réserves couvrent ton solde). Dès qu\\'il y a du disponible, sa répartition apparaîtra ici.</div>';
  const repartHtml=\`<div class="col-12"><div class="card" style="padding:26px 28px;border:1.5px solid var(--glycine);">
    <div style="display:flex;justify-content:space-between;align-items:baseline;flex-wrap:wrap;gap:10px;margin-bottom:4px;">
      <span style="font-size:15px;font-weight:700;color:var(--navy);"><i class="ti ti-arrows-split-2" style="color:var(--bleu);"></i> Où va mon argent — répartition du disponible</span>
      <span style="font-size:13px;color:var(--text-2);">Disponible à répartir <strong style="color:var(--navy);">\${fmt(dispo)}</strong></span>
    </div>
    <div style="font-size:12.5px;color:var(--text-2);margin-bottom:16px;line-height:1.5;">Une fois tes provisions sécurisées (URSSAF, charges…), voici comment se répartit ce qu'il te reste. Le versement tient compte de ce que tu t'es <strong>déjà versé ce mois</strong>. Règles modifiables dans Options.</div>
    \${repInner}
  </div></div>\`;
  el.innerHTML=\`<div class="grid12">
