...
<div id="app">
  ...
  <aside id="sidebar">
    <div class="sidebar-logo">
      ...
      <span style="display:block;font-size:10px;letter-spacing:.04em;color:var(--text-2);opacity:.7;margin-top:2px;">build v124 · Aeration globale (card padding 24->28, grille 20->26, kpi-grid, sections 40/48->44/54, page-header) + pole « Moi » en onglets (Budget perso · Combien me verser · Patrimoine · Projets de vie), menu Personnel resserre a une entree. Navigation / espacements seulement.</span>
    </div>
...
function loadVersement(){
  ...
  // ── Repartition du disponible, adaptee a ce que je me suis deja verse ce mois ──
  let dispo=0; try{const _E=computeEnveloppes();dispo=Math.round((_E.disponible||0)*100)/100;}catch(e){}
  const pctVers=(parseFloat(_sV.pctVersement)||65)/100, pctTreso=(parseFloat(_sV.pctTresorerie)||20)/100, pctForm=(parseFloat(_sV.pctFormation)||10)/100;
  const pctEp=Math.max(0,1-pctVers-pctTreso-pctForm);
  const partVersRaw=dispo>0?Math.round(dispo*pctVers*100)/100:0;
  const partVers=Math.round(Math.min(partVersRaw,conseille)*100)/100;      // plafonne au conseille (#2) : un seul montant de versement de reference
  const surplusVers=Math.max(0,Math.round((partVersRaw-partVers)*100)/100); // ce qui depasse le conseille renforce la tresorerie
  const versementPlafonne=partVersRaw>conseille+0.005;
  const partTreso=Math.round(((dispo>0?dispo*pctTreso:0)+surplusVers)*100)/100;
  const partForm=dispo>0?Math.round(dispo*pctForm*100)/100:0;
  const partEp=dispo>0?Math.round(dispo*pctEp*100)/100:0;
  const resteVirer=Math.max(0,Math.round((partVers-curM.verse)*100)/100);
  ...
  const rcard=(accent,icoColor,ico,titre,gros,sous)=>\`<div style="background:\${accent};border-radius:14px;padding:20px 22px;">
          <div style="font-size:12.5px;color:\${icoColor};font-weight:600;margin-bottom:6px;"><i class="ti \${ico}"></i> \${titre}</div>
          <div style="font-family:'Cormorant Garamond',serif;font-style:italic;font-size:31px;color:var(--navy);line-height:1.1;">\${gros}</div>
          \${sous?\`<div style="font-size:11.5px;color:var(--text-2);margin-top:6px;line-height:1.5;">\${sous}</div>\`:''}
        </div>\`;
  const repInner=dispo>0
    ? \`<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(168px,1fr));gap:18px;">
        \${rcard('var(--glycine)','var(--bleu)','ti-cash','Versement perso',fmt(resteVirer),'reste à te virer ce mois'+(curM.verse>0?' · déjà versé '+fmt(curM.verse):'')+(versementPlafonne?' · <strong>plafonné à ton conseillé '+fmt(conseille)+'</strong>':' · part '+fmt(partVers)))}
        \${rcard('var(--surface-2)','var(--terre-400)','ti-building-bank','Trésorerie',fmt(partTreso),surplusVers>0?'dont <strong>'+fmt(surplusVers)+'</strong> non versé (au-delà de ton conseillé)':'')}
        \${rcard('var(--surface-2)','var(--terre-400)','ti-books','Formation ('+Math.round(pctForm*100)+'%)',fmt(partForm),'')}
        \${pctEp>0.001?rcard('var(--surface-2)','var(--terre-400)','ti-coin','Épargne ('+Math.round(pctEp*100)+'%)',fmt(partEp),''):''}
      </div>\`
    : '<div style="font-size:13.5px;color:var(--text-2);padding:10px 0;line-height:1.6;">Aucun disponible à répartir pour le moment (tes réserves couvrent ton solde). Dès qu\\'il y a du disponible, sa répartition apparaîtra ici.</div>';
  const repartHtml=\`<div class="col-12"><div class="card" style="padding:32px 34px;border:1.5px solid var(--glycine);">
    <div style="display:flex;justify-content:space-between;align-items:baseline;flex-wrap:wrap;gap:12px;margin-bottom:8px;">
      <span style="font-size:16px;font-weight:700;color:var(--navy);"><i class="ti ti-arrows-split-2" style="color:var(--bleu);"></i> Où va mon argent — répartition du disponible</span>
      <span style="font-size:13px;color:var(--text-2);">Disponible à répartir <strong style="color:var(--navy);">\${fmt(dispo)}</strong></span>
    </div>
    <div style="font-size:13px;color:var(--text-2);margin-bottom:22px;line-height:1.6;max-width:640px;">Une fois tes provisions sécurisées (URSSAF, charges…), voici comment se répartit ce qu'il te reste. Le versement est <strong>plafonné à ton conseillé soutenable</strong> ; ce qui dépasse renforce ta trésorerie. Règles modifiables dans Options.</div>
    \${repInner}
  </div></div>\`;
